...
description: A high-performance database interface anchored on near-pure black canvas with light blue as the brand voltage. White typography in confident sans, light blue CTAs, and light-blue-text stat numbers carry the brand voice across every page. Code blocks and product UI fragments embed directly in dark cards. The light blue + black pairing (and light blue used scarcely as accent) is the system's signature — brand identity without atmospheric decoration.

colors:
  primary: "#7dd3fc"
  primary-active: "#38bdf8"
  primary-disabled: "#16313d"
  ink: "#ffffff"
  body: "#cccccc"
  body-strong: "#e6e6e6"
  muted: "#888888"
  ...
  surface-light-blue-band: "#7dd3fc"
  on-primary: "#0a0a0a"
  on-dark: "#ffffff"
  on-light-blue: "#0a0a0a"
...
components:
  ...
  feature-card-light-blue:
    backgroundColor: "{colors.surface-light-blue-band}"
    textColor: "{colors.on-light-blue}"
...
ClickHouse's marketing surface is the highest-contrast interface in the database / data-platform category. The base atmosphere is **near-pure black canvas** (`{colors.canvas}` — #0a0a0a) with **light blue** (`{colors.primary}` — #7dd3fc) as the singular brand voltage. The light blue handles every primary CTA, every stat-callout number, every "GET STARTED" badge — used scarcely on individual elements but generously on full-bleed light blue CTA cards. White typography in confident weight-700 sans-serif anchors the editorial body.

The light blue + black pairing is what makes ClickHouse instantly recognizable. Where Snowflake uses cool blue gradients and Databricks uses red + slate, ClickHouse leans hard into one light blue that does all the brand work. Code blocks, terminal output, and product UI fragments embed directly in dark `{colors.surface-card}` (#1a1a1a) cards across every page.
...
- Electric light blue primary (`{colors.primary}` — #7dd3fc). Used on primary CTAs, large stat-callout numbers ("2.8k+", "74k+"), and full-bleed light blue CTA bands.
...
- Code blocks render in JetBrains Mono inside `{colors.surface-card}`. SQL syntax-highlighted in muted blues / light blues / grays.
- Stat numbers in light blue + sans-700 + huge size carry the credibility moment ("779+", "2.8k+", "47k+" community / contributor / star counts).
...
### Brand & Accent
- **Primary (Light Blue)** (`{colors.primary}` — #7dd3fc): The signature brand color. All primary CTA backgrounds, large stat-callout numbers, full-bleed light blue CTA cards. The light blue is the brand.
- **Primary Active** (`{colors.primary-active}` — #38bdf8): Press / hover-darker variant.
- **Primary Disabled** (`{colors.primary-disabled}` — #16313d): Desaturated dark-blue on dark canvas.
...
- **Surface Light Blue Band** (`{colors.surface-light-blue-band}` — #7dd3fc): The light blue CTA card / band fill — same hex as primary.
...
- **On Primary / On Light Blue** (`{colors.on-primary}` / `{colors.on-light-blue}` — #0a0a0a): Black text on light blue CTAs and light blue CTA bands. The high-contrast light blue + black combo is the brand action signal.
...
| `{typography.stat-display}` | 56px | 700 | 1.0 | -1.5px | Stat callouts ("779+", "47k+") — ALWAYS light blue |
...
| Light Blue band | `{colors.primary}` background — no shadow | Full-bleed light blue CTA cards / bands |
...
- The light-blue-on-black contrast does most of the elevation work for CTAs.
...
**`top-nav`** — Black nav bar pinned to top. 64px tall, `{colors.canvas}` background. Carries the ClickHouse logo + wordmark at left, primary horizontal menu (Product, Use Cases, Pricing, Resources, Customers) center-left, right-side cluster with "Sign in" + "Get Started" `{component.button-primary}` (light blue). Menu items in `{typography.nav-link}` (Inter 14px / 500).

### Buttons

**`button-primary`** — The signature light blue CTA. Background `{colors.primary}` (#7dd3fc), text `{colors.on-primary}` (black), type `{typography.button}` (Inter 14px / 600), padding 12px × 20px, height 40px, rounded `{rounded.md}` (8px). The light blue + black combination is iconic.
...
**`text-link`** — Inline body links in `{colors.primary}` (light blue on dark). Underlined.
...
**`hero-stat-card`** — Light Blue stat-display numbers ("779+", "47k+") inline on the canvas. No card surface — just light blue text in `{typography.stat-display}` (56px / 700).

**`feature-card-light-blue`** — Full-bleed light blue card ("Built for every modern data challenge"). Background `{colors.primary}`, text `{colors.on-light-blue}` (black), rounded `{rounded.lg}` (12px), padding `{spacing.xl}` (32px). The light blue card IS the visual emphasis.
...
**`pricing-tier-card-featured`** — The featured tier flips to `{colors.primary}` (light blue). The light blue surface IS the featured signal.

**`stat-callout`** — Inline light blue stat numbers ("779+", "2.8k+", "47k+"). Transparent background, text `{colors.primary}`, type `{typography.stat-display}`. Used as a flat layout block, not a card with surface.
...
**`text-input-focused`** — Border thickens to `{colors.primary}` (light blue) for emphasis.
...
**`badge-light-blue`** — Light Blue pill for "NEW", "GET STARTED" emphasis. Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.caption-uppercase}`, rounded `{rounded.pill}`.
...
**`cta-band-light-blue`** — A pre-footer "Deploy your way" CTA band. Full light blue fill, black type, rounded `{rounded.lg}`, padding 64px. Carries an h2 in `{typography.display-md}` and a CTA — usually a black-button on the light blue surface.
...
### Do
- Anchor every page on the black canvas. The light blue + black pairing is the brand voltage.
- Reserve `{colors.primary}` (light blue) for primary CTAs, stat-callout numbers, and full-bleed light blue CTA bands. The light blue's scarcity at the element level + abundance at the band level is what makes it powerful.
...
- Use `{component.stat-callout}` numbers to establish credibility (community size, contributors, performance benchmarks). The light blue stat numbers are signature.
- Anchor every band with `{spacing.section}` (96px) vertical rhythm.

### Don't
- Don't introduce a second brand color. ClickHouse is monochromatic + light blue.
...
- Don't use light blue for body text or large surface fills outside of intentional light blue cards.
- Don't use rounded buttons / pills outside of small badges. The standard button radius is 8px (md).
- Don't repeat the same surface mode in two consecutive bands. Black canvas → dark feature card → light blue CTA card → black canvas → code-window card.
...
- Pricing tier cards collapse 4 → 2 → 1; featured tier light blue stays distinct.
...
6. The light blue + black pairing is the brand contract. Don't soften with secondary accents.
7. When in doubt about emphasis: bigger Inter 700 before adding color.

## Known Gaps

- The exact light blue hex (#7dd3fc) was sampled from the screenshot; ClickHouse may publish an official brand color slightly differently.
